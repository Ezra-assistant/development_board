...
u16 mp3_offset_beginning = 0;   //开头偏移多少byte
u16 mp3_offset_ending = 0;      //结尾偏移多少byte
u8 mp3_loopback(void)
{
...
AT(.text.func.music)
void func_music_mp3_res_play(u32 addr, u32 len, u8 loop_cnt)
{
    // u32 cur_time;
    //
    // if (len == 0) {
...
//     mp3_res_play(RES_BUF_MUSIC_MODE_MP3, RES_LEN_MUSIC_MODE_MP3, 0);
// #endif
//     led_music_busy();
//     if ((!fs_mount()) || (!pf_scan_music(1))) {
//         f_msc.cur_dev = sys_cb.cur_dev;
